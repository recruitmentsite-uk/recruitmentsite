/**
 * CLI copy of apps/web social publish helpers (plain ESM, no TS types).
 */
...
function captionFor(platform, body, captions) {
  const fromMap = captions?.[platform]?.trim();
  return fromMap || body.trim();
}

async function publishFacebook({ message, linkUrl }) {
  ...
  if (!token || !pageId) {
    return {
      platform: "facebook",
      status: "skipped",
      error: "META_PAGE_ID / META_PAGE_ACCESS_TOKEN not set",
    };
  }
...
async function publishInstagram({ message, imageUrl }) {
  ...
  if (!token || !igUserId) {
    return {
      platform: "instagram",
      status: "skipped",
      error: "META_IG_USER_ID / META_PAGE_ACCESS_TOKEN not set",
    };
  }
  if (!imageUrl) {
  ...
  const absolute = imageUrl.startsWith("http")
    ? imageUrl
    : `${site.replace(/\/$/, "")}${imageUrl.startsWith("/") ? "" : "/"}${imageUrl}`;
...
async function publishLinkedIn({ message, linkUrl }) {
  ...
  const commentary = linkUrl ? `${message}\n\n${linkUrl}` : message;
  ...
  for (const platform of opts.platforms || []) {
    ...
    try {
      if (platform === "facebook") {
        results.push(await publishFacebook({ message, linkUrl: opts.linkUrl }));
      } else if (platform === "instagram") {
        results.push(await publishInstagram({ message, imageUrl: opts.imageUrl }));
      } else if (platform === "linkedin") {
        results.push(await publishLinkedIn({ message, linkUrl: opts.linkUrl }));
      } else {
        results.push({
          platform,
          status: "skipped",
